...
function roundCommands(cmds, r, round) {
  ...
  subpaths.forEach(subPathCmds => {
    ...
    if (typeof r === 'number') {
      // when r is a number create an array filled with r so that it's always the same input
      r = Array(subPathCmds.length).fill(r);
    } else if (r.length < subPathCmds.length) {
      // when the length of r is less then the commands fill up with a default of 0 to match the length
      r = Object.assign([], Array(subPathCmds.length).fill(0), r);
    }

    // filter the overlap from the subPathCommands and the corners
    subPathCmds = subPathCmds.filter(el => !el.overlap);
    r = r.filter((el, i) => subPathCmds[i] && !subPathCmds[i].overlap);
    ...
    subPathCmds
      .map((el, i, arr) => {
        ...
        const radius = Math.min(r[i], maxRadius);
